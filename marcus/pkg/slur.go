...
func (c *Command) SaySlur() {
	if messageInChannel(c.Session, c.MessageEvent, "general") {
		SendMessageWithError(c.Session, c.MessageEvent, "I can't say slurs in general anymore :x:", "refusing to say a slur")
		c.Session.ChannelMessageSendEmbed(c.MessageEvent.ChannelID, &discordgo.MessageEmbed{
			Title: GetRandomEmbedTitle(),
	...
	go GetAndSpeak(c.Session, c.MessageEvent, content, c.Opts.ChannelName)

	msg := fmt.Sprintf("||```\n%s\n```||", content)
	SendMessageWithError(c.Session, c.MessageEvent, msg, "failed to send message")
}
